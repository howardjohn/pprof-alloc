...
mod enabled {
	...
	use anyhow::Context;
	use libc::{Elf64_Word, PT_LOAD, PT_NOTE, c_int, c_void, dl_iterate_phdr, dl_phdr_info, size_t};

	use super::super::cast::CastFrom;
	use super::BuildId;
					...
					<= orig_offset + usize::cast_from(ph.p_memsz)
				{
						...
						&& usize::cast_from(nh.n_namesz) == GNU_NOTE_NAME.len()
					{
						...
						if name == GNU_NOTE_NAME {
							...
							let desc =
								unsafe { std::slice::from_raw_parts(p_desc, usize::cast_from(nh.n_descsz)) };
